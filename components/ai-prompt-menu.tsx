...
/**
 * Hazır bir istemi kullanıcının seçtiği sohbet modelinde açan menü.
 *
 * Model listesi ve marka ikonları tek yerde: hem günlük menü kartı hem de
 * öğün detayı aynı menüyü kullanıyor, ikisinin farklı model listesine
 * ayrılması bir kopyala-yapıştır hatası olurdu.
 */
...
const MODELS = [
    { key: "chatgpt", label: "ChatGPT", Icon: ChatGptIcon, url: (q: string) => `https://chat.openai.com/?q=${q}` },
    { key: "claude", label: "Claude", Icon: ClaudeBrandIcon, url: (q: string) => `https://claude.ai/new?q=${q}` },
    { key: "grok", label: "Grok", Icon: GrokIcon, url: (q: string) => `https://x.com/i/grok?text=${q}` },
    {
        key: "perplexity",
        label: "Perplexity",
        Icon: PerplexityIcon,
...
/**
 * Menünün açılır olmayan hâli: dört model de tek bakışta, 2x2.
 *
 * Öğün detayında yer var ve kullanıcı zaten bir modalın içinde; üstüne bir
 * açılır menü daha bindirmek yerine seçenekleri doğrudan koymak bir tık
 * kısaltıyor.
 */
export function AiPromptGrid({ prompt, className }: { prompt: string; className?: string }) {
    const encoded = encodeURIComponent(prompt)

    return (
        <div className={cn("grid grid-cols-2 gap-1.5", className)}>
            {MODELS.map(({ key, label, Icon, url }) => (
                <Button
                    key={key}
                    asChild
                    size="sm"
                    variant="outline"
                    className="h-7 justify-start gap-1.5 px-2 text-xs border-border/40"
                >
                    <a
                        href={url(encoded)}
                        target="_blank"
                        rel="noopener noreferrer"
                        title={`${label} ile yorumlat`}
                    >
...
interface AiPromptMenuProps {
    /** Modele gönderilecek hazır istem. */
    prompt: string
    /** Menü başlığının altındaki tek satırlık açıklama. */
    description: string
    align?: "start" | "center" | "end"
    contentClassName?: string
    /** Menüyü açan öğe (asChild ile sarılır). */
